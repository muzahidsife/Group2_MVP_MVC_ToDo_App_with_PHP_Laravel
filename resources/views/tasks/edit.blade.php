...
<div class="max-w-2xl mx-auto">
    ...
    <form method="POST" action="{{ route('tasks.update', $task) }}" class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 space-y-4">
        @csrf @method('PUT')
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                      class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $task->description) }}</textarea>
        </div>
        ...
        <div class="flex justify-end space-x-3 pt-4">
            <a href="{{ route('tasks.show', $task) }}" class="px-4 py-2 text-sm rounded-lg bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 transition">Cancel</a>
            <button type="submit" class="px-6 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition font-semibold">Update Task</button>
        </div>
    </form>
</div>
@endsection
